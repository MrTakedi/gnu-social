...
require INSTALLDIR . "/lib/moduleenableform.php";
require INSTALLDIR . "/lib/moduledisableform.php";
 ...
 * @link     http://status.net/
 */
class ModuleList extends Widget
{
    var $plugins = array();
...
class PluginListItem extends Widget
{
    ...
    protected function getControlForm()
    {
        ...
        if (common_config('plugins', $key)) {
            return new ModuleEnableForm($this->out, $this->plugin);
        } else {
            return new ModuleDisableForm($this->out, $this->plugin);
        }
    }
